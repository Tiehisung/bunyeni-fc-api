...
exports.getTrainingStats = exports.deleteTrainingSession = exports.updateSessionNote = exports.updateAttendance = exports.updateTrainingSession = exports.createTrainingSession = exports.getTrainingSessionById = exports.getPlayerTrainingHistory = exports.getRecentTraining = exports.getUpcomingTraining = exports.getTrainingSessions = void 0;
...
const getTrainingSessions = async (req, res) => {
    try {
        ...
        const search = req.query.training_search || "";
        ...
        const trainingSessions = await training_model_1.default.find(cleaned)
        ...
        const total = await training_model_1.default.countDocuments(cleaned);
        // Get attendance summary
        const attendanceSummary = await training_model_1.default.aggregate([
            {
                $group: {
                    _id: null,
                    totalSessions: { $sum: 1 },
        ...
        res.status(500).json({
            success: false,
            message: (0, lib_1.getErrorMessage)(error, "Failed to fetch training sessions"),
        });
    }
};
exports.getTrainingSessions = getTrainingSessions;
// GET /api/training/upcoming
const getUpcomingTraining = async (req, res) => {
    try {
        const limit = Number.parseInt(req.query.limit || "5", 10);
        const sessions = await training_model_1.default.find({
            date: { $gte: new Date() }
        })
            .populate('recordedBy', 'name email')
            .sort({ date: 1 })
        ...
        res.status(500).json({
            success: false,
            message: (0, lib_1.getErrorMessage)(error, "Failed to fetch upcoming training"),
        });
    }
};
exports.getUpcomingTraining = getUpcomingTraining;
// GET /api/training/recent
const getRecentTraining = async (req, res) => {
    try {
        const limit = Number.parseInt(req.query.limit || "5", 10);
        const sessions = await training_model_1.default.find({
            date: { $lte: new Date() }
        ...
        res.status(500).json({
            success: false,
            message: (0, lib_1.getErrorMessage)(error, "Failed to fetch recent training"),
        });
    }
};
...
const getPlayerTrainingHistory = async (req, res) => {
    try {
        ...
        const sessions = await training_model_1.default.find({
            ...
            .limit(limit)
            .lean();
        const total = await training_model_1.default.countDocuments({
            "attendance.attendedBy._id": playerId
        });
        // Calculate attendance rate
        const totalSessions = await training_model_1.default.countDocuments({
            date: { $lte: new Date() }
        });
        ...
        res.status(500).json({
            success: false,
            message: (0, lib_1.getErrorMessage)(error, "Failed to fetch player training history"),
        });
    }
};
exports.getPlayerTrainingHistory = getPlayerTrainingHistory;
// GET /api/training/:id
const getTrainingSessionById = async (req, res) => {
    try {
        const { id } = req.params;
        const session = await training_model_1.default.findById(id)
            .populate('recordedBy', 'name email')
        ...
        res.status(500).json({
            success: false,
            message: (0, lib_1.getErrorMessage)(error, "Failed to fetch training session"),
        });
    }
};
...
const createTrainingSession = async (req, res) => {
    try {
        const { attendance, date, location, note, recordedBy } = req.body;
        ...
        const existingSession = await training_model_1.default.findOne({
            date: new Date(date),
            location,
        });
        if (existingSession) {
        ...
        // Create session
        const savedSession = await training_model_1.default.create({
        ...
        await (0, helper_1.logAction)({
            ...
            description: `Training session at ${location} on ${(0, timeAndDate_1.formatDate)(date)}`,
        ...
        const populatedSession = await training_model_1.default.findById(savedSession._id)
            .populate('recordedBy', 'name email')
            .populate('attendance.attendedBy', 'name firstName lastName number position')
            .lean();
        res.status(201).json({
        ...
        res.status(500).json({
            message: (0, lib_1.getErrorMessage)(error, "Failed to create training session"),
            success: false,
        });
    }
};
...
const updateTrainingSession = async (req, res) => {
    try {
        ...
        const updated = await training_model_1.default.findByIdAndUpdate(id, {
            $set: {
                ...formData,
                updatedAt: new Date(),
        ...
        res.status(500).json({
            message: (0, lib_1.getErrorMessage)(error, "Failed to update training session"),
            success: false,
        });
    }
};
...
const updateAttendance = async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            message: (0, lib_1.getErrorMessage)(error, "Failed to update attendance"),
        });
    }
};
...
const updateSessionNote = async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            message: (0, lib_1.getErrorMessage)(error, "Failed to update note"),
        });
    }
};
...
const deleteTrainingSession = async (req, res) => {
    try {
        ...
        await training_model_1.default.findByIdAndDelete(id);
        // Log deletion
        await (0, helper_1.logAction)({
            title: "🏋️ Training Session Deleted",
            description: `Training session at ${session.location} on ${(0, timeAndDate_1.formatDate)(session.date)} deleted`,
        ...
        res.status(500).json({
            message: (0, lib_1.getErrorMessage)(error, "Failed to delete training session"),
            success: false,
        });
    }
};
...
const getTrainingStats = async (req, res) => {
    try {
        const stats = await training_model_1.default.aggregate([
            {
                $facet: {
        ...
        res.status(500).json({
            success: false,
            message: (0, lib_1.getErrorMessage)(error, "Failed to fetch training statistics"),
        });
    }
};
